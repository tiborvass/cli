...
// NewGCCommand returns a new cobra prune command for images
func NewGCCommand(dockerCli command.Cli) *cobra.Command {
	options := types.BuildCachePruneOptions{}

	cmd := &cobra.Command{
		Use:   "gc [OPTIONS]",
		Short: "Manage garbage collection rules for build ressources",
		Args:  cli.NoArgs,
